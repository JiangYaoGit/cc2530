...
#define ZG_BUILD_JOINING_TYPE      (ZSTACK_DEVICE_BUILD & (DEVICE_BUILD_ROUTER | DEVICE_BUILD_ENDDEVICE))
  ...
  #define ZG_DEVICE_ROUTER_TYPE 1
#else
  #define ZG_DEVICE_ROUTER_TYPE (zgDeviceLogicalType == ZG_DEVICETYPE_ROUTER)
#endif
...
// Concentrator values
  ...
  #if defined (ZDO_COORDINATOR)
    #if defined (LARGER_NETWORK)              //大型网络为3分钟发送一次MTOR 改为应用发送
  ...
  #if defined (LARGER_NETWORK)          //大网络开启route cadhe
...
extern uint32 zgJoinStartDelay;
#if !defined MT_TASK
...
extern uint8 zgWriteJoinStartupDelay( uint32 joinstartdelay);

extern uint32 zgReadJoinStartupDelay( void );
/*
...
/*
 *  The zb_ReadConfiguration function is used to get a Configuration
 *  Protperty from Nonvolatile memory.
 */
extern uint8 zb_ReadConfiguration( uint8 configId, uint8 len, void *pValue );

/*
 *  The zb_WriteConfiguration function is used to write a
