...
#define MAX_CHANNELS_915MHZ     0x000007FE
  ...
  #if defined(INTRARED_REPEATEER_ROUTER)
...
// Indicate whether to use Multicast in NIB value
  ...
  #define MULTICAST_ENABLED FALSE  // Default to TRUE
#endif

// Maximum number in tables
#if !defined( NWK_MAX_DEVICE_LIST )
  #if defined(INTRARED_REPEATEER_ROUTER)
    #define NWK_MAX_DEVICE_LIST     2  // Maximum number of devices in the
                                        // Assoc/Device list.
  #else
...
// which is the value of the table in the radio
#if defined(INTRARED_REPEATEER_ROUTER)
  #define MAX_NOT_MYCHILD_DEVICES  2
#else
  ...
  #define NWK_MIN_ENDDEVICE_CHILDREN       0
        ...
        #endif
      #else
        #define MAX_NEIGHBOR_ENTRIES    48
      #endif
  #else
    #define MAX_NEIGHBOR_ENTRIES    4
  #endif
...
#define NWK_ADDRESSING_STOCHASTIC     0x02
  ...
  #if defined (LARGER_NETWORK)  		//定义大型网络超过30次丢失LINK STATUS才抛弃
    #define NWK_ROUTE_AGE_LIMIT 30      // 30 missed link satus frames
  #else
    #define NWK_ROUTE_AGE_LIMIT 8       // 3 missed link satus frames
  #endif
#endif
  ...
  #ifndef HAL_BOARD_CC2538
    #if defined(INTRARED_REPEATEER_ROUTER)
      #define MAX_RTG_SRC_ENTRIES 2
    #else
      #if defined (USB_DONGLE)
        #define MAX_RTG_SRC_ENTRIES 25
      #else
        #define MAX_RTG_SRC_ENTRIES 30
      #endif
    #endif
  #else
    #if defined (LARGER_NETWORK) //2014 7 26 定义大型网络SOURCE ROUTER 为300
      #define MAX_RTG_SRC_ENTRIES 150
    #else
      #define MAX_RTG_SRC_ENTRIES 30
	#endif
  #endif
  ...
  #if defined (LARGER_NETWORK)      //定义大型网络老化时间为2S么为什么这么快不理解
    #define SRC_RTG_EXPIRY_TIME 2   // seconds before the source route entry expires
  #else
  ...
  #if defined (LARGER_NETWORK) //2014 7 26 定义大型网络MOTR最短时间为5S
    #define MTO_RREQ_LIMIT_TIME 5000
  #else
    #define MTO_RREQ_LIMIT_TIME 3000
  #endif
#endif
  ...
  #if defined (LARGER_NETWORK) //2014 7 26 定义大型网络超过12次才LINK DOWN
    #define LINK_DOWN_TRIGGER          16  // Link is down if txCounter exceeds this
  #else
  	#define LINK_DOWN_TRIGGER          6   // Link is down if txCounter exceeds this
  #endif
#endif

#if !defined ( GOOD_LINK_COST )
  //#define GOOD_LINK_COST             ( MAX_LINK_COST / 2 )   // 3 is considered good cost
   #define GOOD_LINK_COST      2
#endif
...
// Maximum number of relays in source routing
  ...
  #define MAX_BROADCAST_QUEUED 10
    ...
    #define DEFAULT_ROUTE_REQUEST_RADIUS 10
  #else
...
extern CONST uint8 gMIN_LQI_COST_1;
extern CONST uint8 gMIN_LQI_COST_2;
extern CONST uint8 gMIN_LQI_COST_3;
extern CONST uint8 gMIN_LQI_COST_4;
extern CONST uint8 gMIN_LQI_COST_5;
extern CONST uint8 gMIN_LQI_COST_6;
extern CONST uint8 gMIN_LQI_COST_7;
/*********************************************************************
