...
#ifndef HAL_SB_BOOT_CODE
#include "hal_board_config.h"
#include "nwk.h"
#else
#include "sb_board_cfg.h"
#endif
/* ------------------------------------------------------------------------------------------------
 *                                       Board Indentifiers
 * ------------------------------------------------------------------------------------------------
...
#define VDD_MIN_NV   (VDD_2_0+10) /* 5% margin over minimum to survive a page erase and compaction */
...
{                                                                           \
  ...
  GPIOPinTypeGPIOInput(BSP_LCD_CS_BASE, BSP_LCD_CS); /*CSn input no drive*/ \
                                                                            \
  HAL_CONFIG_RF_FILTER();                                                   \
}
/* ----------- Debounce ---------- */
#define HAL_DEBOUNCE(expr)    { int i; for (i=0; i<500; i++) { if (!(expr)) i = 0; } }

/* ----------- RF Filter ---------- */
#ifdef MAC_RX_ONOFF_FILTER_DEBUG
#define BSP_FILTER_BASE_D            GPIO_D_BASE
#define HAL_CONFIG_RF_FILTER()                          \
{\
  GPIOPinTypeGPIOOutput(BSP_FILTER_BASE_D, GPIO_PIN_2);   \
  GPIOPinTypeGPIOOutput(BSP_FILTER_BASE_D, GPIO_PIN_3);   \
  GPIOPinTypeGPIOOutput(BSP_FILTER_BASE_D, GPIO_PIN_4);   \
  GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_2,0);         \
  GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_3,0);         \
  GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_4,0);         \
}
#define HAL_TURN_ON_RFRX_FILTER()                       \
{                                                       \
  if(_NIB.nwkLogicalChannel <= 13 &&_NIB.nwkLogicalChannel >=11)     \
  {                                                     \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_2,GPIO_PIN_2);\
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_3,0);         \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_4,0);         \
  }                                                     \
  else if(_NIB.nwkLogicalChannel <= 26 &&_NIB.nwkLogicalChannel >=24)\
  {                                                     \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_2,0);         \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_3,GPIO_PIN_3);\
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_4,0);         \
  }                                                     \
  else                                                  \
  {                                                     \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_2,0);         \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_3,0);         \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_4,GPIO_PIN_4);\
 }                                                      \
}
#define HAL_TURN_ON_RFTX_FILTER()                       \
{                                                       \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_2,0);         \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_3,0);         \
    GPIOPinWrite(BSP_FILTER_BASE_D,GPIO_PIN_4,0);         \
