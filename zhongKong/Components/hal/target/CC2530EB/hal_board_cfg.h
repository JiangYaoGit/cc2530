...
#define HAL_SB_NV_ADDR        0x3C800
#define HAL_SB_LOCK_ADDR      0x3F800
...
{                                                                \
  ...
  HAL_CLOCK_CONFIG();                        /* Select 32MHz XOSC and the source for 32K clock */ \
  HAL_CLOCK_STABLE();                        /* Wait for the change to be effective */   \
...
/* ----------- RF Filter ---------- */
#if defined(WL_R0042E) && defined(MAC_RX_ONOFF_FILTER_DEBUG)
#define HAL_CONFIG_RF_FILTER()                          \
{                                                       \
  MAC_RX1_TURN_OFF();                                   \
  MAC_RX2_TURN_OFF();                                   \
  MAC_RX1_DDR |= MAC_RX1_BV;                            \
  MAC_RX2_DDR |= MAC_RX2_BV;                            \
}
...
#define VDD_2_5  93   // 2.0 V required to safely read/write internal flash.
#define VDD_2_3  84   // 2.0 V required to safely read/write internal flash.
#define VDD_2_7  100  // 2.7 V required for the Numonyx device.

#define VDD_MIN_RUN   VDD_2_3
#define VDD_MIN_NV   (VDD_2_0+5)  // 5% margin over minimum to survive a page erase and compaction.
