...
Wulian, Inc.
----------
# Version 1.1.3-honeywell #
September 27, 2016

## Notices: ##

 - [1] 提交更新了新的版本的基础库,用于honeywell的1(coordinator)+2(Router)+30(EndDevice)项目专用基础库

## Changes:##

 - [1] 增加 WZCL_SendAsWithAckMessage的API

 - [2] 网关路由Hold时间改为65s

 - [3] 路由设备使用RF发射功率为20dBm的定制lib

 - [4] 改善ZigBee网络的路径选择

 - [5] 心跳规律改为前三小时1min/次，三小时后10min/次，通过RS来标记心跳变化规律，RS:XXXXXXXXXXX/01表示1min一次心跳  RS:XXXXXXXXXXX/02表示10min一次心跳，直接到网关的心跳为1/3的心跳周期时间  

 - [6] PollRate规律改为，前5min 5s/次，接下来30min 18s/次，35min后不发PollRate，直接靠心跳的reponse Pollrate，心跳Response PollRate时间改为1.5s
 
 - [7] 增加void WZCL_ActiveDevice(void)的API,只要调用该API,PollRate规律按照[6]描述的规律重新开始

 - [8] 应用实现每隔1h调用一次来上报双向信号值

## Bug Fixes: ##

 - [1] 修复网关设备在其子设备EndDevice的信号值不好时选择从路由到网关子设备的路径造成数据发送失败的问题；

## Known Issues: ##
 - [1] 设备入网后重新上电偶尔会出现丢失RS信息的问题。；

 - [2] 路由会出现绿灯一直常亮的情况，在设备请求双向信号值时abs(信号值)<10时。
